...
def cloud_synthesize(text, speaker_id, api_key=""):
    ...
    if api_key == "":
        ...
        url = (
            f"{VOICE_VOX_URL_HIGH_SPEED}?text={text}&speaker={speaker_id}&key={api_key}"
        )
...
def start_STTS_pipeline(use_chatbot=False):
    ...
    try:
        ...
        if input_language_name != "English" and not translator.use_deepl:
            task = "translate"
...
def start_TTS_pipeline(input_text):
    ...
    log_message(f"Speech synthesized for text [{filtered_text}] ({step_timer.end()}s)")
    log_message(
