...
class Config(customtkinter.CTkToplevel):
    def __init__(self):
        ...
        label_gpu = customtkinter.CTkLabel(
            master=self, text=f'Using GPU: {SUB.check_gpu_status()}')
        label_gpu.pack(padx=20, pady=10)
...
thread_recording = Thread(target=SUB.start_recording_loop)
thread_recording.start()
thread_transcription = Thread(target=SUB.start_transcription_loop)
